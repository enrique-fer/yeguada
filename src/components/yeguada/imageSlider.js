...
class ImageSlider extends Component {
  ...
  render() {
    ...
    const prev =
      this.state.current === 0 ? this.state.length - 1 : this.state.current - 1;
    const next =
      this.state.current === this.state.length - 1 ? 0 : this.state.current + 1;
...
function mapSliderData(sliderData, imgIndex, type, handleClick) {
  return sliderData.map((slide, index) => {
    return (
      <div
        className={`slide ${index === imgIndex ? type : ""}`}
        key={index}
