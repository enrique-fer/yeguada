...
var controller = {
    ...
    save: (req, res) => {
        ...
        if (validate_activity_id && validate_date &&
            validate_start && validate_end && validate_user_id) {
            ...
            Usuario.findById(params.user_id)
                .then(user => {
                    usuario = user;
                    Actividad.findById(params.activity_id)
                        .then(activity => {
                            actividad = activity
                            reserva.save((error, reservaStored) => {
                                ...
                                message = `${usuario.nombre} ${usuario.apellidos} a creado una reserva para ${actividad.title}, el ${reservaStored.date.getUTCDate() + 1}/${reservaStored.date.getMonth() + 1}/${reservaStored.date.getFullYear()}, a las ${params.start}`;
                                ...
                                message = `Tienes una reserva para ${activity.title}, el ${reservaStored.date.getUTCDate() + 1}/${reservaStored.date.getMonth() + 1}/${reservaStored.date.getFullYear()}, a las ${params.start}`;
